...
export default function PasswordStep({ value, onChange, setIsStepValid }: PasswordStepProps) {
  ...
  return(
    <ContentLayout>
      ...
      <DefaultInput label='회원가입하고 책마루에 가입하세요' placeholder='10자리 ~ 20자리, 영문, 특수문자 포함' isError={false} warningMessage="" value={value} onChangeText={onChange} />
      <DefaultInput label='' placeholder='비밀번호 재입력' isError={value.length > 0 && value !== checkPassword} warningMessage="비밀번호가 일치하지 않습니다." value={checkPassword} onChangeText={setCheckPassword} />
    </ContentLayout>
  )
}